"""
Answer-only training: Qwen3-14B with reasoning as visible output

Trains the model on step-by-step reasoning as the visible answer
(no <think> block), then evaluates with native thinking enabled.
The hypothesis: training on reasoning may override the model's native
(potentially better) thinking with shorter/weaker reasoning.

Configuration:
    - Model: Qwen3-14B (4-bit quantized via Unsloth)
    - LoRA: r=16, alpha=16
    - Training: 2 epochs, lr=1e-5, enable_thinking=False
    - Eval: temp=0.6 sampling, enable_thinking=True (native)
    - Judge: Claude 3.5 Sonnet via OpenRouter
    - Data format: reasoning + answer as visible output (no <think>)

Result: 70.18% ± 16.04% (5-fold CV on 53 samples)
"""
...
EVAL_THINKING_MODE = True   # Enable native thinking at inference
...
Provide a detailed step-by-step solution to the user's problem.
Use rigorous mathematical reasoning and LaTeX for formulas.
...
# Data preparation — answer-only (key difference)


def build_training_texts(examples, tokenizer, rng=None, log_tokens=False):
    """
    Format training samples with reasoning as VISIBLE answer (no <think> block).

    The step-by-step reasoning from Claude Opus becomes the model's direct output.
    At inference, enable_thinking=True lets the model also use its native thinking.
    ...
    for q, r, a in zip(questions, reasonings, answers):
        step_by_step_answer = f"{r}\n\n**Final Answer:** {a}"
        messages = [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": q},
            {"role": "assistant", "content": step_by_step_answer},
        ]
        text = tokenizer.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=False,
            enable_thinking=False,  # No thinking block in training
        )
        texts.append(text)
...
def generate_answer(model, tokenizer, question):
    """Generate an answer with native thinking enabled (even though training had it off)."""
...
def evaluate_fold(model, tokenizer, dataset, fold_num):
    ...
    out_path = f"eval_details_answer_only_fold_{fold_num + 1}.json"
...
if __name__ == "__main__":
    ...
    results_file = f"evaluation_results_answer_only_{run_timestamp}.json"
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(full_dataset)))):
        ...
        trainer = SFTTrainer(
            ...
            args=SFTConfig(
                ...
                seed=SEED,
                logging_steps=5,
                output_dir=f"outputs_answer_only_fold_{fold}",
                save_strategy="no",
                report_to="none",
            ),
        )
        ...
        json_results = {
            "metadata": {
                ...
                "training_format": "reasoning as visible answer, no <think> block",
    ...
    print(f"ANSWER-ONLY COMPLETE — Mean: {np.mean(fold_accuracies):.2f}% ± {np.std(fold_accuracies):.2f}%")
